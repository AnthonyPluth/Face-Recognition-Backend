...
class FaceDetector_DLIBCNN:
    ...
    def detect(self, frame):
        ...
        faces_updated = []
        for face in faces:
            (x, y, w, h) = (face.rect.left(), face.rect.top(), face.rect.right() -
                            face.rect.left(), face.rect.bottom()-face.rect.top())
            faces_updated.append((x, y, w, h))
        return faces_updated
...
class FaceDetector_SSDRESNET:
    ...
    def __init__(self, path, optimize, minfacesize):
        ...
        self._detector = cv2.dnn.readNetFromCaffe(
            path + 'deploy.prototxt', path + 'res10_300x300_ssd_iter_140000.caffemodel')

    def detect(self, frame):
        if self._optimize:
            imageBlob = cv2.dnn.blobFromImage(cv2.resize(
                frame, (150, 150)), 1.0, (50, 50), (104.0, 177.0, 123.0), swapRB=False, crop=False)
        else:
            imageBlob = cv2.dnn.blobFromImage(cv2.resize(
...
class FaceDetector_CORAL:
    ...
    def __init__(self, path, optimize, minfacesize):
        ...
        self._detector = DetectionEngine(
            path + 'mobilenet_ssd_v2_face_quant_postprocess_edgetpu.tflite')
    ...
    def detect(self, frame):
        ...
        faces = self._detector.detect_with_image(
            frame, threshold=0.05, keep_aspect_ratio=True, relative_coord=False, top_k=10)
        faces_updated = []
        for face in faces:
