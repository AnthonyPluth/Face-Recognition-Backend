...
class FaceDetector:
    def __init__(
        self,
        model=FaceDetectorModels.DEFAULT,
        path=None,
        optimize=False,
        minfacesize=20,
    ):
...
class FaceDetector_HAARCASCADE:
    ...
    def detect(self, frame):
        ...
        return self._detector.detectMultiScale(
            frame_gray, 1.1, 5, minSize=(self._minfacesize, self._minfacesize)
        )
...
class FaceDetector_SSDRESNET:
    ...
    def detect(self, frame):
        ...
        for index in range(faces.shape[2]):
            ...
            if confidence > 0.50:
                box = faces[0, 0, index, 3:7] * np.array(
